...
const _initBoardArray = () => {
    ...
    gameState = 0;
    flagCount = 0;
    gamingStatusEle.innerHTML = 'Ready';
...
const revealCell = (row, col, callback = false) => {
    ...
    if (gameState === 0) {
        ...
        _startTimer();
        gameState = 1;
        gamingStatusEle.innerHTML = 'Gaming';
    }
...
const _explode = () => {
    gameState = 2;
    _stopTimer();
    gamingStatusEle.innerHTML = 'Exploded';
    for (const [row, col] of getAllCoordinates(settings.rows, settings.cols)) {
        if (_isMine(row, col)) {
            boardMask[row][col] = 1;
            _refreshDisplay(row, col);
...
const check = () => {
    if (_whetherWin()) {
        gameState = 2;
        _stopTimer();
        gamingStatusEle.innerHTML = 'Clear';
    }
}
...
const _tryMinePlaces = () => {
    ...
    if (minesLeft <= 0 || minesLeft >= unrevealedBlocksCount) {
        return {
            hints: [],
            reason: "Bad number of remaining mines"
        };
    }
    ...
    return {
        hints: mineSets,
        reason: `Worked out by trying ${Math.round(possibilities)} possibilities`
    };
}
...
const _getHintPlaces2 = () => {
...

    {
        ...
        if (out.minePos.length || out.safePos.length) {
            ...
            out.reason = 'Lucky you! You run into a dilemma! <br>(Wait 3 secs...)';
            setTimeout(() => {
                ...
                document.querySelector('#err_help').innerHTML = 'Solved the dilemma for you~'
            }, 3000);
            return out;
        }
    }
...
const resetCallback = () => {
    ...
    if (!/^\d+$/.test(new_row)) { alert("Rows must be positive integer! "); return; }
    if (!/^\d+$/.test(new_col)) { alert("Cols must be positive integer!"); return; }
    if (!/^\d+$/.test(new_col)) { alert("Mines must be positive integer!"); return; }
    let rows = +new_row;
    let cols = +new_col;
    let mines = +new_mines;
    if (!(5 <= rows <= 1000)) { alert("Rows must be bigger than 5! "); return; }
    if (!(5 <= cols <= 1000)) { alert("Cols must be bigger than 5! "); return; }
    if (mines <= 0) { alert("Mines must be positive! "); return; }
    if (mines > rows * cols - 9) { alert("Too many mines to put in grid! "); return; }
    settings.rows = rows;
    settings.cols = cols;
...
const helpCallback = () => {
    ...
    err_help.innerHTML = '';
    if (gameState === 0) {
        err_help.innerHTML = "Click on the board to start the game!"
        return;
    } else if (gameState === 2) {
        err_help.innerHTML = "Click the reset button to restart. "
        return;
    } else if (possViewStat) {
        err_help.innerHTML = "Click anywhere to exit probability view.  "
        return;
    } else {
