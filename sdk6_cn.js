...
const title = (() => {
    ...
    let ct = document.createTextNode("数独(杰版)");
...
const promptDisplayer = (() => {
    let ele = document.createElement('div');
    let ct = document.createTextNode('是新游戏喔');
    ele.appendChild(ct);
    bodyAppend([ele]);
    return ele;
})();
...
class SodukuDetailedSolver extends SodukuSolver {
    ...
    *fillCertain() {
        ...
        while (pls.length && pls[0][1].length <= 1) {
            ...
            let v = pls[0][1][0];
            this.set(t, v);
            this.hint_soleNumber(`杰哥的房子(${t[0] + 1}列${t[1] + 1}行)蛮大的，但他(${v})一个人住。`, t);
            yield;
            this.display();
            this.poss.set(t, undefined);
            pls.shift();
            ...
            for (let i in [1, 2, 3, 4, 5, 6, 7, 8, 9]) {
                if (o.filter((item) => (item == i)).length == 1) {
                    for (let t of l) {
                        if (this.poss.get(t) && arrayInclude(this.poss.get(t), i)) {
                            this.set(t, i);
                            this.hint_solePlace(`俏佳人(${i})只能去杰哥家(${t[0] + 1}列${t[1] + 1}行)。`, t, l);
                            yield;
                            this.display();
                            this.poss.set(t, undefined);
                            out = true;
    ...
    *guess() {
        ...
        this.hint_general(`很多俏佳人[${l.join('/')}]都去杰哥家(${p[0] + 1}列${p[1] + 1}行)，
            但只有阿伟${this.answer.data.get(p)}超勇。`, p);
        yield;
        this.display();
        return;
    }
    ...
    *_call() {
        while (true) {
            ...
            if (this.complete()) {
                promptDisplayer.innerText = '点击退出好康模式'
                yield;
                return this;
            }
            {
...
class Game extends Soduku {
    ...
    clear() {
        ...
        promptDisplayer.innerText = '是新游戏喔？'
    }
    ...
    enterDemo() {
        ...
        promptDisplayer.innerText = "好康模式：点击帮助杰哥找到俏佳人";
    }
...
class SodukuHint extends SodukuSolver {
    ...
    fillCertain() {
        ...
        while (pls.length && pls[0][1].length <= 1) {
            if (!pls[0][1].length) {
                ...
                promptDisplayer.innerText = '杰哥不要！！！'
                throw new NoSolution();
            }
            let t = pls[0][0];
            let v = pls[0][1][0];
    ...
    guess() {
        ...
        this.hint_general(`杰哥(${p[0] + 1}列${p[1] + 1}行)常常帮助一些俏佳人[${l.join('/')}]。`, p);
        display(this.source);
        return;
    }
    ...
    _call() {
        if (this.checkIllegal()) {
            ...
            promptDisplayer.innerText = "吾知汝学堂亦知汝班，握着我的抱枕"
            buttons.hint.disabled = "disabled";
            return;
        }
        try {
...
buttons.pause.onclick = () => {
    ...
    if (m.style.opacity === '') {
        m.style.opacity = '0';
        g.frozen = true;
        buttons.pause.innerText = '打电动';
        promptDisplayer.innerText = '已拒绝访问';
    } else {
        m.style.opacity = '';
        g.frozen = false;
        buttons.pause.innerText = '休息一下';
        promptDisplayer.innerText = '玩♂得开心';
    }
}
buttons.reset.onclick = () => {
    if (confirm("阿伟勇不勇？")) {
        promptDisplayer.innerText = '杰哥正在脱下眼镜';
        setTimeout(() => {
...
buttons.demo.onclick = () => { if (confirm("彬彬逊不逊？")) g.enterDemo(); }
